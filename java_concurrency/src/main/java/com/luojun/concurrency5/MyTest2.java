...
public class MyTest2 {
    ...
    public static void main(String[] args) {
        CyclicBarrier cyclicBarrier = new CyclicBarrier(3, () -> {
            System.out.println(Thread.currentThread().getName() + " break barrier action");
            // 在最后一个动作到达屏障之后，
            // 获取到每个线程所持有的部分结果，然后进行最后结果的汇总、处理、加工。
        });

        for (int i = 0; i < 5; ++i) {       // 两个执行阶段（屏障）
            for (int n = 0; n < 3; ++n) {   // 三个执行线程
                new Thread(() -> {
                    try {
                        ...
                        System.out.println(Thread.currentThread().getName() + " hello " + randomInt);
                        // 线程到达屏障
                        cyclicBarrier.await();

                        System.out.println(Thread.currentThread().getName() + " world " + randomInt);
